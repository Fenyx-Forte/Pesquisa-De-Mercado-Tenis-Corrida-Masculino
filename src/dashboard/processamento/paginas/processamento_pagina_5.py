...
from dashboard.processamento.queries import pagina_5_queries
...
def calcular_df_hoje(conexao: DuckDBPyConnection) -> pd_DataFrame:
    query = pagina_5_queries.query_faixas_preco_hoje()
...
def calcular_df_periodo(
    ...
    query = pagina_5_queries.query_faixas_preco_periodo()
...
def dados_abaixo_de_200(df: pd_DataFrame) -> pd_DataFrame:
    return df[df["faixa_preco"] == "200"].sort_values(
...
def dados_entre_200_e_400_(df: pd_DataFrame) -> pd_DataFrame:
    return df[df["faixa_preco"] == "200_400"].sort_values(
        by=["num_produtos", "marca"],
        ascending=[False, True],
    )
