def query_numero_produtos_hoje() -> str:
    query = """
    SELECT
...
def query_media_produtos_periodo() -> str:
    query = """
    WITH produtos_por_dia AS (
        SELECT
            COUNT(*) as numero_produtos
        FROM
...
def query_numero_produtos_em_promocao_hoje() -> str:
    query = """
    SELECT
...
def query_media_produtos_em_promocao_periodo() -> str:
    query = """
    WITH produtos_em_promocao_por_dia AS (
        SELECT
            COUNT(*) as numero_produtos_em_promocao
        FROM
            dados_completos_por_periodo($data_inicio, $data_fim)
...
def query_percentual_medio_desconto_hoje() -> str:
    query = """
    SELECT
...
def query_media_percentual_desconto_periodo() -> str:
    query = """
    WITH percentual_desconto_por_dia AS (
        SELECT
    ...
    FROM
        percentual_desconto_por_dia;
    """

    return query


def query_media_precos_hoje() -> str:
    query = """
    SELECT
...
def query_media_precos_periodo() -> str:
    query = """
    ...
    SELECT
        FORMAT('{:.2f}', AVG(media_precos)) as media_precos
    FROM
        precos_por_dia;
    """
