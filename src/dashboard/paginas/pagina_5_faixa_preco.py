...
from dashboard.processamento.paginas import processamento_pagina_5
...
register_page(
    ...
    name="Faixa Preço",
    title="Faixa Preço",
    description="Página Faixa Preço",
...
def modal_erro() -> Modal:
    conteudo = Modal(
        [
            ModalHeader(ModalTitle("Titulo", id="pagina_5_modal_erro_titulo")),
            ModalBody("Conteudo", id="pagina_5_modal_erro_conteudo"),
        ],
        id="pagina_5_modal_erro",
        is_open=False,
    )
...
def tabela(
    ...
    conteudo = AgGrid(
        rowData=dados,
        id=f"pagina_5_tabela_{sufixo_coluna}_{sufixo_informacao}",
    ...
    conteudo = html.Div(
        [
            ...
            html.Div(
                ...
                id=f"pagina_5_periodo_{sufixo_coluna}",
            ),
            html.Br(),
            div_informacao(
                subtitulo_coluna="Preço Abaixo de R$200",
                dados=dados_abaixo_de_200,
                sufixo_coluna=sufixo_coluna,
                sufixo_informacao="200",
            ),
            div_informacao(
                subtitulo_coluna="Preço Entre R$200 e R$400",
            ...
            html.Br(),
            div_informacao(
                subtitulo_coluna="Preço Acima de R$400",
...
def linha_colunas() -> Row:
    ...
    lista_dados_hoje = gerenciador.pagina_5_inicializa_dados_hoje()

    lista_dados_escolhido = gerenciador.pagina_5_inicializa_dados_escolhido()

    lista_dados_historico = gerenciador.pagina_5_inicializa_dados_historico()
...
clientside_callback(
    processamento_pagina_5.callback_verificar_datas(),
    Output("pagina_5_modal_erro_titulo", "children"),
    Output("pagina_5_modal_erro_conteudo", "children"),
    Input("pagina_5_botao", "n_clicks"),
    State("pagina_5_seletor_datas", "start_date"),
    State("pagina_5_seletor_datas", "end_date"),
    State("pagina_5_periodo_hoje", "children"),
    State("pagina_5_periodo_escolhido", "children"),
    State("pagina_5_periodo_historico", "children"),
    prevent_initial_call=True,
)


clientside_callback(
    processamento_pagina_5.callback_abrir_modal(),
    Output("pagina_5_modal_erro", "is_open"),
    Input("pagina_5_modal_erro_titulo", "children"),
    prevent_initial_call=True,
)


@callback(
    Output("pagina_5_tabela_escolhido_200", "rowData"),
    Output("pagina_5_tabela_escolhido_200_400", "rowData"),
    Output("pagina_5_tabela_escolhido_400", "rowData"),
    Output("pagina_5_seletor_datas", "start_date"),
    Output("pagina_5_seletor_datas", "end_date"),
    Output("pagina_5_periodo_escolhido", "children"),
    Input("pagina_5_modal_erro_titulo", "children"),
    State("pagina_5_seletor_datas", "start_date"),
    State("pagina_5_seletor_datas", "end_date"),
    prevent_initial_call=True,
)
def pagina_5_atualiza_dados_periodo_escolhido(titulo, data_inicio, data_fim):
    if titulo != "":
        raise PreventUpdate

    periodo_novo = processamento_pagina_5.retorna_periodo_novo(
        data_inicio, data_fim
    )

    lista_dados = gerenciador.pagina_5_atualiza_dados_escolhido(
        data_inicio, data_fim
    )
