...
select marcas_unicas.marca
from
  '../dados/marcas_unicas/marcas_unicas.csv' as marcas_unicas;

create function tratar_string(coluna) as trim(
  regexp_replace(
    regexp_replace(
      replace(
        regexp_replace(
...
create function adicionar_preco_completo(
  col_preco_reais, col_preco_centavos
) as cast(
  (
    case
      when
        col_preco_reais is not null
        and col_preco_centavos is not null
        then concat(
...
fase_1 as (
  select
    cast(coalesce(df.nota_avaliacao, '0') as decimal(4, 2))
      as nota_avaliacao,
    cast(
      coalesce(
        trim(
    ...
    cast(df._data_coleta as date) as data_coleta,
    cast(df._data_coleta as time) as horario_coleta,
    coalesce(tratar_string(df.marca), 'GENERICA') as marca,
    coalesce(tratar_string(df.produto), 'PRODUTO SEM NOME') as produto,
    adicionar_preco_completo(
      df.preco_velho_reais, df.preco_velho_centavos
    ) as preco_velho,
    adicionar_preco_completo(
      df.preco_atual_reais, df.preco_atual_centavos
    ) as preco_atual,
